...
local Library, UISettings, Modules = {}, {}, {}
Library.Notification = {}; Modules.Notify = Library.Notification
...
local UIS = SafeGetService("UserInputService") or game:GetService("UserInputService")
...
local CoreGui = (gethui and gethui()) or SafeGetService("CoreGui") or game:GetService("CoreGui")
...
Modules.CoreGui = CoreGui

local Notification = Modules.Notify
local TweenInfo, _ENV = TweenInfo.new, Modules.ENV
...
local SaveSystem = Modules.Save
function SaveSystem:RegisterKey(key) self._Keys[key] = true end
...
function SaveSystem:PruneStaleKeys()
	...
	local slots = self:Get("_saveSlots", {}); local slotChanged = false
...
function SaveSystem:GetPlayerName()
	...
	if not lp then pcall(function() lp = game:GetService("Players").LocalPlayer end) end
	local name = lp and lp.Name
	if not name or name == "" then
		name = "User_" .. tostring(lp and lp.UserId or "0")
	end
	return name:gsub("[^%w]", "")
end
...
function SaveSystem:GetGameId()
	local gid = 0
	pcall(function() gid = game.GameId end)
	if not gid or gid == 0 then
		pcall(function() gid = game.PlaceId end)
	end
...
Modules.Util, Modules.Tween, Modules.Media, Modules.Theme, Modules.UI = {}, {}, {}, {}, {}
local Util, Tw, Media, ThemeMod = Modules.Util, Modules.Tween, Modules.Media, Modules.Theme
...
function Util.MakeDraggable(topBar, body)
...
function Tw.Info(duration, style, direction, repeatCount, reverses, delayTime)
	return TweenInfo(duration, style or Enum.EasingStyle.Cubic, direction or Enum.EasingDirection.Out, repeatCount or 0, reverses or false, delayTime or 0)
end
...
function Media.PathToAsset(path)
	...
	local customAssetFn = getcustomasset or (getgenv and getgenv().getcustomasset) or getsynasset or (getgenv and getgenv().getsynasset)
	if customAssetFn then
		...
		if SaveSystem and SaveSystem.FolderName then
			local folderPath1 = SaveSystem.FolderName .. "/" .. trimmed
			ok, asset = pcall(customAssetFn, folderPath1)
			if ok and type(asset) == "string" and asset ~= "" then return asset end

			local folderPath2 = SaveSystem.FolderName .. "\\" .. trimmed
...
function Media.IsVideo(path)
	...
	local lower = string.lower(path):gsub("%s+$", "")
	return lower:find("%.webm") ~= nil or lower:find("%.mp4") ~= nil or lower:find("%.mov") ~= nil or lower:find("%.mkv") ~= nil or lower:find("%.avi") ~= nil or lower:find("%[vid%]") ~= nil
end

local Creator = Util.Create
local Protect_UI, GetRandomString, setProperty = Util.Protect, Util.RandomName, Util.Set
...
function Library:DestroyGui()
	if self._CurrentGui and self._CurrentGui.Parent then self._CurrentGui:Destroy(); self._CurrentGui = nil end
end
function CircleClick(Button, X, Y)
	task.spawn(function()
		...
		local Circle = Creator("ImageLabel", {
			...
			Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0, NewX, 0, NewY),
		}, Button)
...
function ThemeMod.Color(name)
	if ThemeManager.Current and ThemeManager.Current[name] then return ThemeManager.Current[name] end
...
function Notification:Init()
	if not self.GUI then
		...
		screenGui.Name = GetRandomString()
...
Notification.ActiveMessages = Notification.ActiveMessages or {}
function Notification:Notify(NofDebug, MiddleDebug)
	...
	local RawButtons = NofDebug.Buttons or NofDebug.Button or NofDebug.Actions or NofDebug.Options or (MiddleDebug and (MiddleDebug.Buttons or MiddleDebug.Actions))
	local ButtonsList = {}
	...
	local ButtonAreaHeight = HasButtons and (ButtonHeight + 8) or 0

	local WrappedText = WrapText(Description, Font, FontSize, MaxWidth)
	local MeasuredTextHeight = GetWrappedTextHeight(WrappedText, Font, FontSize, MaxWidth)
	local TotalHeight = 28 + MeasuredTextHeight + ButtonAreaHeight + 14
	local FullWidth = 200
	local PillWidth = 46
	local PillHeight = 26

	local Host = Creator("Frame", {
		AnchorPoint = Vector2.new(1, 0),
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
		Position = UDim2.new(1, -10, 0, 10),
		Size = UDim2.new(0, PillWidth, 0, 0),
		ClipsDescendants = false,
		ZIndex = 1000
	}, self.GUI)
	...
	local DescLabel = Creator("TextLabel", {
		...
		Size = UDim2.new(0, 184, 0, MeasuredTextHeight),
		ZIndex = 1002,
		Font = Font,
		Text = WrappedText,
		TextColor3 = Color3.fromRGB(180, 180, 180),
	...
	local PillLogo = Creator("ImageLabel", {
		Name = "PillLogo",
	...
	local DoneLabel = Creator("TextLabel", {
		Name = "DoneLabel",
		AnchorPoint = Vector2.new(0.5, 0.5),
		BackgroundTransparency = 1,
		Position = UDim2.new(0.5, 0, 0.5, 0),
	...
	local ProgressBarBackground = Creator("Frame", {
		...
		["Children"] = {
			...
			Creator("Frame", {
				...
				["Children"] = {
					Creator("UICorner", { CornerRadius = UDim.new(1, 0) }),
					Creator("UIGradient", { Color = ThemeColor("Lit"), Rotation = 0 })
				}
			})
		}
	})
	...
	if HasButtons then
		ButtonContainer = Creator("Frame", {
			...
			BackgroundTransparency = 1,
			Position = UDim2.new(0, 8, 0, 22 + MeasuredTextHeight + 6),
			Size = UDim2.new(1, -16, 0, ButtonHeight),
			ZIndex = 1002,
			["Children"] = {
				Creator("UIListLayout", {
					Name = "ButtonsLayout",
		...
		for idx, btnData in ipairs(ButtonsList) do
			...
			if isPrimary == nil then
				local lower = string.lower(btnText)
				if numButtons == 1 or lower == "okay" or lower == "ok" or lower == "yes" or lower == "confirm" or lower == "accept" or lower == "apply" then
					isPrimary = true
				else
					isPrimary = false
				end
			...
			local btnBg = isPrimary and Color3.fromRGB(38, 38, 38) or Color3.fromRGB(28, 28, 28)
			local btnHoverBg = isPrimary and Color3.fromRGB(52, 52, 52) or Color3.fromRGB(40, 40, 40)
			local btnTextColor = isPrimary and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(180, 180, 180)

			local btnStroke = Creator("UIStroke", {
				Color = isPrimary and Color3.fromRGB(80, 80, 80) or Color3.fromRGB(48, 48, 48),
				Transparency = 1,
				Thickness = 1,
	...
	local notifChildren = {
		Creator("UICorner", { CornerRadius = UDim.new(0, 13) }),
		PillLogo,
		DoneLabel,
		TitleLabel,
		CooldownLabel,
	...
	local fillBar = ProgressBarBackground.ProgressBarFill
	...
	local function DismissNotification()
		...
		UISettings:Tween(TitleLabel, { TextTransparency = 1 }, 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
		UISettings:Tween(CooldownLabel, { TextTransparency = 1 }, 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
		UISettings:Tween(DescLabel, { TextTransparency = 1 }, 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
		UISettings:Tween(ProgressBarBackground, { BackgroundTransparency = 1 }, 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
		UISettings:Tween(fillBar, { BackgroundTransparency = 1 }, 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
		for _, b in ipairs(createdButtons) do
			UISettings:Tween(b.Instance, { BackgroundTransparency = 1, TextTransparency = 1 }, 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
			if b.Stroke then UISettings:Tween(b.Stroke, { Transparency = 1 }, 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In) end
		end
		task.wait(0.2)

		UISettings:Tween(Host, {
			Size = UDim2.new(0, PillWidth, 0, PillHeight)
		}, 0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
		task.wait(0.28)

		UISettings:Tween(DoneLabel, { TextTransparency = 0 }, 0.18, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		task.wait(0.35)

		UISettings:Tween(Host, {
			Size = UDim2.new(0, 0, 0, 0)
		}, 0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
		UISettings:Tween(DoneLabel, { TextTransparency = 1 }, 0.12, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
		task.wait(0.18)
	...
	for _, b in ipairs(createdButtons) do
		...
		local btnStrokeInst = b.Stroke
		local btnData = b.Data
		btnInst.MouseEnter:Connect(function()
			if not isDismissed then
				Tw.Play(btnInst, { BackgroundColor3 = b.HoverBg }, 0.15, Enum.EasingStyle.Quint)
				if btnStrokeInst then Tw.Play(btnStrokeInst, { Transparency = 0.5 }, 0.15) end
			end
		end)
		btnInst.MouseLeave:Connect(function()
			if not isDismissed then
				Tw.Play(btnInst, { BackgroundColor3 = b.DefaultBg }, 0.2, Enum.EasingStyle.Quint)
				if btnStrokeInst then Tw.Play(btnStrokeInst, { Transparency = 0.8 }, 0.2) end
			end
		end)
		btnInst.MouseButton1Click:Connect(function()
			if isDismissed then return end
			CircleClick(btnInst, Mouse.X, Mouse.Y)
	...
	UISettings:Tween(PillLogo, { Size = UDim2.new(0, 18, 0, 18), ImageTransparency = 0 }, 0.24, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, function()
		UISettings:Tween(PillLogo, { ImageTransparency = 1, Size = UDim2.new(0, 0, 0, 0) }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
		UISettings:Tween(Host, {
			Size = UDim2.new(0, FullWidth, 0, TotalHeight)
		}, 0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, function()
			UISettings:Tween(TitleLabel, { TextTransparency = 0 }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
			UISettings:Tween(CooldownLabel, { TextTransparency = 0 }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
			UISettings:Tween(DescLabel, { TextTransparency = 0 }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
			UISettings:Tween(ProgressBarBackground, { BackgroundTransparency = 0 }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
			UISettings:Tween(fillBar, { BackgroundTransparency = 0 }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
			for _, b in ipairs(createdButtons) do
				UISettings:Tween(b.Instance, { BackgroundTransparency = 0, TextTransparency = 0 }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
				if b.Stroke then UISettings:Tween(b.Stroke, { Transparency = 0.8 }, 0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out) end
			end
...
function TranslateText(target, text)
...
function RegisterTranslatable(instance, text)
...
function ApplyTranslation(langCode)
	if TranslationData._busy then
		TranslationData._pending = langCode
		return
	end
	TranslationData._busy = true
	TranslationData._pending = nil

	local function finish()
		TranslationData._busy = false
		if TranslationData._pending and TranslationData._pending ~= langCode then
			local nextLang = TranslationData._pending
			TranslationData._pending = nil
			task.defer(ApplyTranslation, nextLang)
		end
	end
	...
	if langCode == "en" then
		for instance, data in pairs(TranslationData.RegisteredElements) do
			if instance and instance.Parent then
				pcall(function() instance.Text = data.original end)
			end
		end
		finish()
		return
	...
	task.spawn(function()
		...
		local function ApplyText(original, translated)
			for instance, data in pairs(TranslationData.RegisteredElements) do
				if instance and instance.Parent and data.original == original then
					pcall(function() instance.Text = translated end)
				end
			end
		end

		for i, text in ipairs(queue) do
			local key = langCode .. ":" .. text
			local translated = TranslationData.Cache[key]
			local wasCached = translated ~= nil
			if not translated then
				translated = TranslateText(langCode, text)
				TranslationData.Cache[key] = translated
			end
			ApplyText(text, translated or text)
...
function Library:CreateWindow(config)
	...
	function F.IsDeveloperBuild()
		local v = string.lower(tostring(Version))
		return v == "developer" or v == "v.developer" or string.find(v, "developer", 1, true) ~= nil
	end
	...
	function F.HasKeyAccess()
		if not KeyConfig then return true end
		return KeyValid
	end

	local dummyObj
	function F.CreateDummy()
		local dummy = {}
		setmetatable(dummy, {
	...
	function RegisterKeyLocked(frame, locked)
		...
		table.insert(KeyLockedElements, {
			frame = frame,
			blocker = blocker,
		})
	end
	...
	function RegisterThemeElement(element, prop, role)
		table.insert(LiveThemeElements.accentElements, { element, prop, role or "Accent" })
	end
	function RegisterLitGradient(grad)
		table.insert(LiveThemeElements.litGradients, grad)
	end
	function RegisterButtonGradient(grad)
		table.insert(LiveThemeElements.buttonGradients, grad)
	end
	...
	function UnscaledLayout(size)
		local scale = MainUIScale.Scale
		if scale == 0 then return size end
		return size / scale
	end

	function FitScrollCanvas(scroll, layout, axis, pad)
		...
		view = UnscaledLayout(math.max(view, 0))
		if content > 0 then content = content + pad end
		local canvas = math.max(content, 0)
	...
	function RegisterLayoutRefresh(fn)
		table.insert(LayoutRefreshRegistry, fn)
	end

	function ScaledFontSize(base, minBase)
		local s = MainUIScale.Scale
		if s <= 0 then s = 1 end
		if s < 1 then
			return math.max(math.floor(base * s), 4), 1
		end
		local boost = math.min(math.floor((s - 1) * 10 + 0.5), 6)
		return math.min(base + boost, 18), 1
	end
	...
	function F.SetUIScalePreview(scale)
		scale = math.clamp(scale, UIScaleMin, UIScaleMax)
	...
	local function UpdateAutoScale()
		...
		local scaleRatio = math.min(size.X / refWidth, size.Y / refHeight, 1.0)
		local targetScale = math.clamp(UserSetScale * scaleRatio, UIScaleMin, UIScaleMax)
		...
		if isVid then
			...
			if BodyVideo then
				...
				if not _videoLoopThread then
					_videoLoopThread = task.spawn(function()
						while true do
							task.wait(1.5)
	...
	if savedBg ~= "" then
		task.defer(function() ApplyBackgroundImage(savedBg) end)
	end
	...
	function F.MakeMiniSlider(parent, labelText, settingKey, min, max, defaultVal, step, onChange, zBase, deferApply)
		...
		Creator("TextButton", {
			...
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.new(0.5, 0, 0.5, 0),
			Size = UDim2.new(0, math.max(thumbSize + 14, 28), 0, math.max(thumbSize + 14, 28)),
			Text = "",
			ZIndex = zBase + 5,
		}, Thumb)
	...
	function F.SetCanvasHeight(count)
		local full = LIST_PAD + count * ITEM_H + math.max(count - 1, 0) * PADDING
		...
		for order, name in ipairs(filtered) do
			...
			Item.MouseButton1Click:Connect(function()
				...
				if GetAutoTranslate() then
					...
					if code then
						task.spawn(ApplyTranslation, code)
					end
				end
			end)
		end
	...
	DeletePanelBtn.MouseButton1Click:Connect(function()
		...
		table.remove(slots, OpenedSlotIndex)
		SaveSystem:Save("_saveSlots", slots)
	...
	function RefreshSlots()
		...
		if #SaveSlots == 0 then
			...
			table.insert(SlotFrames, emptyLabel)
		end
		...
		for i, slot in ipairs(SaveSlots) do
			...
			table.insert(SlotFrames, slotRow)
		end
	end
	...
	function F.BuildThemeCards()
		...
		for _, tName in ipairs(themeOrder) do
			...
			table.insert(ThemeCardFrames, card)
	...
	function F.BuildBgImageCards()
		...
		local media = {}
		for _, p in ipairs(images) do table.insert(media, p) end
		for _, p in ipairs(videos) do table.insert(media, p) end
	...
	function F.ActivateScroll(TargetScrollFrame, TargetTabButton, TargetUnderline, SkipAnim)
		...
		if not SkipAnim then
			task.delay(0.15, function()
				...
				task.delay(0.28, function() transitioning = false end)
			end)
		else
	...
	function F.RefreshSectionVisibility()
		...
		for _, v in ipairs(SearchRegistry) do
			if not Seen[v.sectionScroll] then
				...
				table.insert(SectionScrolls, v.sectionScroll)
			end
		end
		...
		for _, x in ipairs(SearchRegistry) do
			...
			if not already then table.insert(list, x.sectionScroll) end
		end
	...
	function F.ScoreSearchEntry(entry, query)
		...
		if title:sub(1, #q) == q then score = math.max(score, 160) end
		if title:find(q, 1, true) then score = math.max(score, 120) end
		if menu:find(q, 1, true) then score = math.max(score, 90) end
		if tab:find(q, 1, true) then score = math.max(score, 70) end
		...
		if #words > 1 then
			...
			if allHit then score = math.max(score, 100 + wordScore) end
		end

		if F.fuzzyMatch(title, query) then score = math.max(score, 45) end
		if F.fuzzyMatch(path, query) then score = math.max(score, 35) end
		return score
	end
	...
	function F.NavigateToSearchEntry(entry)
		...
		task.defer(function()
			...
			local targetY = math.max(0, rel - 40)
			...
			if not stroke then
				...
				task.delay(0.8, function() if stroke and stroke.Parent then stroke:Destroy() end end)
			else
				local oldT = stroke.Transparency
				stroke.Transparency = 0.15
				task.delay(0.8, function() if stroke and stroke.Parent then stroke.Transparency = oldT end end)
			end
		end)
	end
	...
	function F.UpdateSearchTreeUI(matches, query)
		...
		for _, entry in ipairs(matches) do
			...
			table.insert(grouped[tabKey][menuKey], entry)
		end
		...
		local viewH = math.max(UnscaledLayout(SearchTreeScroll.AbsoluteSize.Y), 1)
	...
	function F.UpdateHistoryUI(showTree)
		...
		if showTree then
			...
			local viewH = math.min(treeH, 180)
	...
	function DoSearch(query)
		...
		SearchCountBadge.Size = UDim2.new(0, math.max(22, #tostring(matchCount) * 8 + 10), 0, 14)
	...
	function Tabs:AddTab(Tab_Title, Tab_Icon, condition)
		...
		local Section = {}
		function Section:addSection(condition)
			if not checkCondition(condition) then return dummyObj end
			...
			local Menus = {}
			function Menus:addMenu(Menu_Title, condition)
				if not checkCondition(condition) then return dummyObj end
				...
				local function SectionSize()
					local Height = math.max(UnscaledLayout(SectionListLayout.AbsoluteContentSize.Y) + 4, 22)
				...
				local function RegisterElement(elementFrame, titleText)
					...
					local function togglePin()
						...
						if idx then
							...
							table.insert(list, entry.favKey)
				...
				function Funcs:addButton(Title_Button, callback, locked, condition, liteAllowed)
					if not checkCondition(condition) then return dummyObj end
					...
					MainButton.MouseEnter:Connect(function()
						...
						UISettings:Tween(Arrow,
							{ Position = UDim2.new(1, -9, 0.5, 0), TextColor3 = Color3.fromRGB(216, 180, 254) }, 0.2,
					...
					MainButton.MouseLeave:Connect(function()
						...
						UISettings:Tween(Arrow,
							{ Position = UDim2.new(1, -12, 0.5, 0), TextColor3 = Color3.fromRGB(192, 132, 252) }, 0.25,
							Enum.EasingStyle.Quint)
					end)
					MainButton.MouseButton1Click:Connect(function()
						...
						task.delay(0.1, function()
							UISettings:Tween(Arrow, { Position = UDim2.new(1, -9, 0.5, 0) }, 0.25, Enum.EasingStyle.Back,
								Enum.EasingDirection.Out)
						end)
						callback()
					end)
			...
			function Funcs:addTextbox(titleText, callback, adjustableTitle, savekey, condition, liteAllowed)
				...
				Funcs.AddButton = Funcs.addButton
				...
				Funcs.AddKeybind = Funcs.addKeybind
				Funcs.AddLine = Funcs.addLine
				Funcs.AddParagraph = Funcs.addParagraph
	...
	do
		...
		task.defer(function()
				...
				for _, t in ipairs(TabRegistry) do
					if t.tabButton and t.tabButton.Text == "Main" and t.scrollFrame then
						...
						for _, ch in ipairs(t.scrollFrame:GetChildren()) do
							if ch.Name == "SectionScroll" then table.insert(scrolls, ch) end
						end
			...
			if FavoritesAPI.Refresh then FavoritesAPI.Refresh() end
			task.delay(1.5, function() if FavoritesAPI.Refresh then FavoritesAPI.Refresh() end end)
		end)
